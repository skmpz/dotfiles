...
sudo pkg install -y bash cmake gcc synergy freecolor autocutsel plexmediaserver uncrustify gmake rsync scrot ImageMagick7 e2fsprogs pkgconf wget unrar python2 python3 py27-pip py36-pip pulseaudio alsa-utils mate-terminal rxvt-unicode urxvt-perls gtk-arc-themes fusefs-ntfs >> .install.log 2>> .install.log
check $?
